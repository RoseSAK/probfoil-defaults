        ...
        with Timer(category="enqueue"):
            ...
            if not rule.literal:
                ...
                if rule.literal.arguments == rule.target.arguments:
                    ...
                    for ex_id, example in rule.enum_examples():
                        fact_id = self.grounding.getFact(
                            str(Literal(rule.literal.functor, example)))
                        if fact_id == None:
            ...
            with Timer(category="grounding_generateprogram"):
                ...
                for rule_id, rule_exids in enumerate(self.toGround):
                    ...
                    if RPF:
                        ...
                        gp.append(
                            query + ':-' + str(Literal(clause_pred, rule.target.arguments)) + '.')
    ...
    def _ground(self, program):
        if program:
            ...
            with Timer(category='grounding_grounding') as tmr:
                ...
                ground_program = self._call_grounder(pl_filename)

            with Timer(category='grounding_integrating') as tmr:
                # 3) Read in grounding and insert new things into Grounding data structure
                return self.grounding.integrate(ground_program)
        else:
            return {}, []

    def _call_grounder(self, in_file):
        pl = problog.program.PrologFile(in_file)
        ground_program = problog.formula.LogicFormula.create_from(pl)
            ...
            with Timer(category='evaluate_compiling'):
                ...
                with Timer('Compiling %s' % cnf[0], verbose=self.env['verbose'] > 1):
                    if USE_C2D:
                        ...
                        cmd = [executable, '-dt_method', '0', '-reduce', '-visualize', '-in',
                               cnf_file]
                    ...
                    with open(os.devnull) as null:
                        ...
                        if retcode:
                            ...
                            if self.env['verbose'] > 3:
                                print('WARNING: compilation interrupted, memory exceeded',
                                      self.env['memlimit'], retcode)
                            return None
...
class Grounding(object):
    ...
    def integrate(self, lines, rules=None):
        ...
        for line_type, line_content, line_alias in lines[1:]:
            line_num += 1
            node_id = self._integrate_line(line_num, line_type, line_content, line_alias, lines,
                                           ln_to_ni, rules)
            if node_id != None:
        ...
        if line_type == 'fact':
            ...
            for subnode in line_content:
                if type(subnode) == tuple:
                    subnodes.append(
                        self._integrate_line(None, subnode[0], subnode[1], None, lines, ln_to_ni,
                                             rules))
                else:
                    ...
                    tr_subnode = self._integrate_line(subnode_id, subnode[0], subnode[1],
                                                      subnode[2], lines, ln_to_ni, rules)
                    if neg:
    ...
    def stats(self):
        return namedtuple('IndexStats', ('atom_count', 'name_count', 'fact_count'))(len(self), 0,
                                                                                    len(
                                                                                        self.__fact_names))

    def __str__(self):
        return '\n'.join('%s: %s (p=%s)' % (i + 1, n, self.__probabilities[i]) for i, n in
                         enumerate(self.__nodes))
